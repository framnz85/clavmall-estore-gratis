...
import { deleteAdminOrder } from "../../functions/order";
...
const OrderTable = ({ loading }) => {
  ...
  const handleDelete = (order) => {
    deleteAdminOrder(estoreSet._id, order._id, user.token).then((res) => {
      if (res.data.err) {
    ...
    },
    {
      ...
      content: (order) => {
        return (
          ...
          >
            <Link to={`/${estoreSet.slug}/admin/dashboard/${order._id}`}>
              <UnorderedListOutlined />
            </Link>
